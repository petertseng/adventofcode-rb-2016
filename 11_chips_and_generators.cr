...
if has_flag?('t')
  testinput = [
    [(0_u8 << TYPE_BITS) | CHIP, (1_u8 << TYPE_BITS) | CHIP],
    [(0_u8 << TYPE_BITS) | GENERATOR],
    [(1_u8 << TYPE_BITS) | GENERATOR],
    [] of Component,
  ]

  solve.call(testinput, {"hydrogen" => 0_u8, "lithium" => 1_u8})
else
  ...
  solve.call(realinput, elements)

  realinput[0].concat((0_u8...4_u8).map { |x| x + (elements.size << TYPE_BITS) })
