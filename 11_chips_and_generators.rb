...
if has_flag?(?t)
  testinput = [
    [(0 << TYPE_BITS) | CHIP, (1 << TYPE_BITS) | CHIP],
    [(0 << TYPE_BITS) | GENERATOR],
    [(1 << TYPE_BITS) | GENERATOR],
    [],
  ]

  solve[testinput, {'hydrogen' => 0, 'lithium' => 1}]
else
  ...
  solve[realinput, elements]

  realinput[0].concat((0...4).map { |x| x + (elements.size << TYPE_BITS) })
